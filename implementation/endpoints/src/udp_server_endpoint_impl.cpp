    ...
    if (_multicast_id == multicast_id_ && multicast_socket_ && multicast_socket_->is_open()) {
        auto its_storage = std::make_shared<udp_endpoint_receive_op::storage>(
                ...
                std::bind(&udp_server_endpoint_impl::on_multicast_received,
                          ...
                          std::placeholders::_4),
                &multicast_recv_buffer_[0], max_message_size_, _multicast_id, is_v4_,
...
bool udp_server_endpoint_impl::send_queued(const target_data_iterator_type _it) {
    ...
    if (its_entry.second > 0) {
        if (last_sent_ != std::chrono::steady_clock::time_point()) {
            const auto its_elapsed = std::chrono::duration_cast<std::chrono::microseconds>(
                                             std::chrono::steady_clock::now() - last_sent_)
                                             .count();
    ...
    unicast_socket_->async_send_to(
            ...
            [its_me, _it, its_entry](boost::system::error_code const& _error, std::size_t _bytes) {
...
void udp_server_endpoint_impl::get_configured_times_from_endpoint(
    ...
    configuration_->get_configured_timing_responses(
            _service, udp_server_endpoint_base_impl::local_.address().to_string(),
...
void udp_server_endpoint_impl::leave_unlocked(const std::string& _address) {
    ...
    try {
        if (is_joined(_address)) {
            VSOMEIP_DEBUG << "Leaving the multicast group " << _address << " from "
                          << local_.address().to_string();

            if (multicast_socket_) {
                auto its_endpoint_host = endpoint_host_.lock();
                if (its_endpoint_host) {
                    multicast_option_t its_leave_option {shared_from_this(), false,
        ...
        VSOMEIP_ERROR << __func__ << ":" << e.what() << " address: " << _address;
    }
}
...
void udp_server_endpoint_impl::on_multicast_received(boost::system::error_code const& _error,
...
void udp_server_endpoint_impl::on_message_received(boost::system::error_code const& _error,
                                                   std::size_t _bytes, bool _is_multicast,
        ...
        if (!_error && 0 < _bytes) {
            ...
            do {
                uint64_t read_message_size =
                        utility::get_message_size(&_buffer[i], remaining_bytes);
                ...
                if (current_message_size > VSOMEIP_SOMEIP_HEADER_SIZE
                    ...
                    if (remaining_bytes - current_message_size > remaining_bytes) {
                        ...
                        return;
                               ...
                               && (_buffer[i + VSOMEIP_PROTOCOL_VERSION_POS]
                                           != VSOMEIP_PROTOCOL_VERSION
                                   || !utility::is_valid_message_type(tp::tp::tp_flag_unset(
                                           _buffer[i + VSOMEIP_MESSAGE_TYPE_POS]))
                                   || !utility::is_valid_return_code(static_cast<return_code_e>(
                        ...
                        if (_buffer[i + VSOMEIP_PROTOCOL_VERSION_POS] != VSOMEIP_PROTOCOL_VERSION) {
                            ...
                            its_host->on_message(&_buffer[i], VSOMEIP_SOMEIP_HEADER_SIZE + 8, this,
                    ...
                    if (tp::tp::tp_flag_is_set(_buffer[i + VSOMEIP_MESSAGE_TYPE_POS])) {
                        ...
                        if (its_instance != ANY_INSTANCE) {
                            if (!tp_segmentation_enabled(its_service, its_instance, its_method)) {
                                VSOMEIP_WARNING
                                        ...
                                        << " which is not configured for TP:"
                                        << " local: " << get_address_port_local()
                                        << " remote: " << its_remote_address << ":" << std::dec
                                        << its_remote_port;
                                return;
                        ...
                        const auto res = tp_reassembler_->process_tp_message(
                                &_buffer[i], current_message_size, its_remote_address,
                                its_remote_port);
                        if (res.first) {
                            if (utility::is_request(res.second[VSOMEIP_MESSAGE_TYPE_POS])) {
                                const client_t its_client = bithelper::read_uint16_be(
                        ...
                        if (its_service != VSOMEIP_SD_SERVICE
                            || (current_message_size > VSOMEIP_SOMEIP_HEADER_SIZE
                                && current_message_size >= remaining_bytes)) {
                            its_host->on_message(&_buffer[i], current_message_size, this,
                                                 _is_multicast, VSOMEIP_ROUTING_CLIENT, nullptr,
                                                 its_remote_address, its_remote_port);
                        } else {
                            // ignore messages for service discovery with shorter SomeIP length
                            VSOMEIP_ERROR << "Received an unreliable vSomeIP SD message with too "
                                             "short length field"
                                          << " local: " << get_address_port_local()
                                          << " remote: " << its_remote_address << ":" << std::dec
                                          << its_remote_port;
                    ...
                    if (remaining_bytes > VSOMEIP_SERVICE_POS_MAX) {
                        service_t its_service =
                                bithelper::read_uint16_be(&_buffer[VSOMEIP_SERVICE_POS_MIN]);
                        if (its_service != VSOMEIP_SD_SERVICE) {
                            if (read_message_size == 0) {
                                VSOMEIP_ERROR << "Ignoring unreliable vSomeIP message with SomeIP "
                                                 "message length 0!";
                            } else {
                                auto its_endpoint_host = endpoint_host_.lock();
                                if (its_endpoint_host) {
                                    its_endpoint_host->on_error(
                                            &_buffer[i], (uint32_t)remaining_bytes, this,
                                            its_remote_address, its_remote_port);
                                }
                            }
                        }
...
void udp_server_endpoint_impl::print_status() {
    ...
    VSOMEIP_INFO << "status use: " << std::dec << local_port_ << " number targets: " << std::dec
                 ...
                 << unicast_recv_buffer_.capacity() << " multicast_recv_buffer: " << std::dec
...
void udp_server_endpoint_impl::set_multicast_option(const boost::asio::ip::address& _address,
    ...
    if (_is_join) {
        ...
        ::setsockopt(multicast_socket_->native_handle(), (is_v4_ ? IPPROTO_IP : IPPROTO_IPV6),
                     (is_v4_ ? IP_PKTINFO : IPV6_PKTINFO), its_pktinfo_option,
        ...
        multicast_socket_->set_option(
                boost::asio::socket_base::receive_buffer_size(its_udp_recv_buffer_size), _error);
        if (_error) {
            return;
        }
#ifndef _WIN32
        ...
        if (setsockopt(multicast_socket_->native_handle(), SOL_SOCKET, SO_RCVTIMEO, &timeout,
                       sizeof(timeout))
            == -1) {
            VSOMEIP_WARNING << __func__ << ": unable to setsockopt SO_RCVTIMEO";
        }

        if (setsockopt(multicast_socket_->native_handle(), SOL_SOCKET, SO_SNDTIMEO, &timeout,
                       sizeof(timeout))
            == -1) {
            VSOMEIP_WARNING << __func__ << ": unable to setsockopt SO_SNDTIMEO";
        }
#endif
        boost::asio::socket_base::receive_buffer_size its_option;
...
#endif
        VSOMEIP_INFO << "udp_server_endpoint_impl<multicast>: SO_RCVBUF is: " << std::dec
                     << its_option.value() << " (" << its_udp_recv_buffer_size
                     << ") local port:" << std::dec << local_port_;
